...
export async function getDomains(db: D1Database): Promise<Domain[]> {
  return db.prepare('SELECT * FROM domains ORDER BY created_at DESC').all<Domain>().then(r => r.results);
}

export async function addDomain(db: D1Database, domain: string): Promise<void> {
  await db.prepare('INSERT OR IGNORE INTO domains (domain) VALUES (?)').bind(domain.toLowerCase()).run();
}

export async function deleteDomain(db: D1Database, domain: string): Promise<void> {
  await db.prepare('DELETE FROM domains WHERE domain = ?').bind(domain.toLowerCase()).run();
}
